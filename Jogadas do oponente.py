...
while jogando==True:
    ...
    while True:
        linha_inimiga=random.randint(0,9)
        coluna_inimiga=random.randint(0,9)
        if [linha_inimiga,coluna_inimiga] not in ataques:
            ataques.append([linha_inimiga,coluna_inimiga])
            break
    print(f'Seu oponente está atacando na linha {linha_inimiga} e coluna {coluna_inimiga}')
    tabuleiro_jogador=faz_jogada(tabuleiro_jogador,linha_inimiga,coluna_inimiga)
    monitoria_jogador=afundados(frota_jogador,tabuleiro_jogador)
    if monitoria_jogador==10:
